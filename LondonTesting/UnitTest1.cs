...
public class Tests
{
    ...
    [Test]
    public void Should_Return_Right_Board_When_Found()
    {
        //Arrange
        var boards = new List<BoardDTO>()
        {
        ...
        //Assert
        Assert.That(result.GetType(), Is.EqualTo(typeof(OkObjectResult)));
        Assert.That(((OkObjectResult)result).Value, Is.EqualTo(boards.Find(b => b.Id == 1)!));
    }

    [Test]
    public void Should_Return_NotFound_When_Not_Found()
    {
        //Arrange
        var boards = new List<BoardDTO>()
        {
        ...
        var mockService = new Mock<IBoardService>();
        mockService
        .Setup(serivce => serivce.GetBoard(4))
        .Throws(new Exception("Not found"));

        var controller = new BoardController(mockService.Object);

        //Act
        var result = controller.GetBoard(4);
    ...
    [Test]
    public void Should_Delete_Right_Board_When_Exists()
    {
        //Arrange
        var boards = new List<BoardDTO>()
        {
        ...
        var boardToRemove = boards.Find(b => b.Id == 1);

        var mockService = new Mock<IBoardService>();
        mockService
        .Setup(serivce => serivce.RemoveBoard(boardToRemove.Id));
        boards.Remove(boards.Find(b => b.Id == boardToRemove.Id));
        mockService.Setup(service => service.GetAllBoards()).Returns(boards);

        var controller = new BoardController(mockService.Object);

        //Act
        var removeResult = controller.DeleteBoard(boardToRemove.Id);
        var boardsResult = controller.GetAllBoards();

        //Assert
        Assert.That(removeResult.GetType(), Is.EqualTo(typeof(OkResult)));
        Assert.That(boardsResult.GetType(), Is.EqualTo(typeof(OkObjectResult)));
        Assert.That(((List<BoardDTO>?)((OkObjectResult)boardsResult).Value).Contains(boardToRemove), Is.EqualTo(false));
    }


    [Test]
    public void Should_Return_NotFound_When_Deleting_Not_Existing_Board_With_3Boards()
    {
        //Arrange
        var boards = new List<BoardDTO>()
        {
        ...
        var mockService = new Mock<IBoardService>();
        mockService
        .Setup(serivce => serivce.RemoveBoard(4))
        .Throws(new Exception("Not Found"));

        var controller = new BoardController(mockService.Object);

        //Act
        var removeResult = controller.DeleteBoard(4);

        //Assert
        Assert.That(removeResult.GetType(), Is.EqualTo(typeof(NotFoundResult)));
    }
}